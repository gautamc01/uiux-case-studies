> 🚀 Designed a seamless water tanker booking experience to solve real-world water accessibility issues.

# 💧 Neer Prakalp – Water Tanker Delivery App
## 📄 Full Case Study PDF
[View Complete Case Study](./Water%20Tanker%20UIUX%20Case%20Study.pdf)

## 📌 Project Overview
Neer Prakalp is a mobile application designed to simplify and streamline water tanker booking for residential, commercial, and industrial users. It connects users with local water suppliers, ensuring reliable, transparent, and efficient water delivery.

## ⏱ Duration
7 Weeks

## 🎯 Problem Statement
Current water supply systems are unreliable and inefficient, causing inconsistent availability and lack of transparency.

## 🎯 Goal
Design a user-friendly solution that enables:
- Real-time tanker booking  
- Transparent pricing  
- Order tracking  
- Efficient supplier management  

## 🧠 My Role
- User Research  
- UX Design  
- Wireframing & Prototyping  
- User Testing  

## 🛠 Tools
Figma, Adobe Illustrator, Google Forms, Miro

---

## 🔍 User Research
Conducted qualitative and quantitative research to understand:
- Difficulty in finding nearby suppliers  
- Lack of real-time tracking  
- Unreliable delivery  

---

## 👤 User Persona
**Raj Vishwanath (Restaurant Owner, Pune)**  
Needs reliable water supply to run business without interruption.

---

## 💡 Key Insights
- Users need **fast & reliable delivery**
- Transparency builds trust
- Emergency booking is critical

---

## 🔄 User Flow
(We’ll add image here next step)

---

## 🧱 Information Architecture
(We’ll add image here next step)

---

## ✏️ Wireframes
(We’ll add image here next step)

---

## 🎨 Final UI Screens

### 🏠 Home Experience
![Home Screen](Home%20Screen%201.png)

---

### 📦 Browse & Select
![Product Screen](Product%20Screen%201.png)

---

### 💳 Checkout Flow
![Payment Screen](Payment%20Screen%201.png)

---

### ✅ Order Confirmation
![Success Screen](Success%20screen%201.png)

## 💡 Key UX Decisions
- Simplified booking flow for faster ordering  
- Clear pricing to build user trust  
- Real-time updates to reduce uncertainty  
---

## 📈 Outcome
- Improved booking experience  
- Reduced user frustration  
- Better transparency in water delivery  

---

## 🚀 Future Scope
- Water quality tracking  
- AI-based demand prediction  
- Smart notifications  

---

## 🙌 Thanks
If this project resonates with you, feel free to connect!
